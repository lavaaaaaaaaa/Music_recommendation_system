...
def recommend_songs(song_name, df, num_recommendations=5):
    ...
    # Check if the song exists (case-insensitive)
    if song_name_lower not in df['name_lower'].values:
        raise ValueError("Song not found")

    # Get the cluster of the input song
    song_cluster = df[df["name_lower"] == song_name_lower]["Cluster"].values[0]
    same_cluster_songs = df[df["Cluster"] == song_cluster]
    song_index = same_cluster_songs[same_cluster_songs["name_lower"] == song_name_lower].index[0]
